...
win_label = Label(root, text="WIN").grid(row=3, column=1)
...
win_entry = Entry(root, textvariable=win_entry_data, width=5).grid(row=4, column=1)
...
def get_ideia_data():
    ...
    with open('trade_info_win.txt', 'w') as f:
        f.write(f'{id_trade}\n')
...
frame2_label = Label(root, text="Ideia de Trade", 
                     font=("Arial", 15)).grid(row=5, column=0, columnspan=4)
...
frame2_label = Label(root, text="Ideia Atual", 
                    font=("Arial", 15))
frame2_label.grid(row=10, column=0, columnspan=4)
...
def send_messages_trade_idea():
    ...
    with open('trade_info_win.txt', 'r', newline='') as f:
