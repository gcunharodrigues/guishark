...
win_label = Label(root, text="WIN").grid(row=3, column=2)
...
renko_label = Label(root, text="Renko:").grid(row=4, column=0)
wdo_entry_data = StringVar()
wdo_entry = Entry(root, textvariable=wdo_entry_data ,width=5).grid(row=4, column=1)
win_entry_data = StringVar()
win_entry = Entry(root, textvariable=win_entry_data, width=5).grid(row=4, column=2)
...
def get_ideia_data():
    ...
    wdo_renko = wdo_entry_data.get()
    win_renko = win_entry_data.get()
    id_trade = id_entry_data.get()
    ativo_trade = ativo2_menu.get()
    tipo_trade = tipo_menu.get()
    entrada_trade = entrada_entry_data.get()
    
    stop, alvo_1, alvo_2, alvo_3 = trade_idea_v1.send_trade_idea(int(wdo_renko), 
                                  int(win_renko), 
                                  ativo_trade, 
                                  tipo_trade, 
                                  float(entrada_trade),
    ...
    with open('trade_info.txt', 'w') as f:
        f.write(f'{id_trade}\n')
...
def send_messages_trade_idea():
    ...
    with open('trade_info.txt', 'r', newline='') as f:
